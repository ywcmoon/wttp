 ...
 *   2. 支持卡片多选（勾选/取消勾选），选中后高亮显示
 *   3. 显示每个卡片的探索进度（已探索 / 进度百分比）
 *   4. 确认选择后跳转到 goLink.html 开始探索
 *
 * 模块结构：
 *   一、全局状态变量：选中集合、选项卡、数据缓存
 *   二、工具函数：选中计数更新
 *   三、核心业务逻辑：卡片渲染、选中交互、弹窗控制、探索确认
 *   四、事件绑定：按钮事件初始化
 *   五、页面初始化入口：DOMContentLoaded
 *
 * 数据流：
 ...
 *   - exploreBlocks：探索卡片数据（兼容旧格式）
 *   - exploreProgress：探索进度记录
 *   - ls2：选中的卡片 ID 列表
 *   - selectedExploreCards：选中的卡片 ID（兼容旧键名）
 *   - selectedCardsDetail：选中卡片的详情数据
 *
 * 依赖：
